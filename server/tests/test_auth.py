...
class TestRegister(unittest.TestCase):
    ...
    def test_creates_user_and_returns_cookie(self):
        ...
        resp = _run(handle_register(body))
    ...
    def test_duplicate_username_rejected(self):
        username = f"tu_{uuid.uuid4().hex[:8]}"
        body = json.dumps({"username": username, "password": "secret123"}).encode()
        _run(handle_register(body))
        resp = _run(handle_register(body))
        self.assertEqual(resp.status_code, 409)

    def test_empty_username_rejected(self):
        resp = _run(handle_register(json.dumps({"username": "", "password": "secret"}).encode()))
        self.assertEqual(resp.status_code, 400)

    def test_short_password_rejected(self):
        resp = _run(handle_register(json.dumps({"username": "testuser", "password": "ab"}).encode()))
        self.assertEqual(resp.status_code, 422)

    def test_invalid_json(self):
        resp = _run(handle_register(b"not json"))
        self.assertEqual(resp.status_code, 400)
...
class TestLogin(unittest.TestCase):
    ...
    def setUp(self):
        ...
        _run(handle_register(json.dumps({"username": self.username, "password": "secret123"}).encode()))

    def test_valid_credentials(self):
        resp = _run(handle_login(json.dumps({"username": self.username, "password": "secret123"}).encode()))
        self.assertEqual(resp.status_code, 200)
        self.assertIn("set-cookie", resp.headers)

    def test_wrong_password(self):
        resp = _run(handle_login(json.dumps({"username": self.username, "password": "wrongpass"}).encode()))
        self.assertEqual(resp.status_code, 401)

    def test_nonexistent_user(self):
        resp = _run(handle_login(json.dumps({"username": "nobody_long_random_name", "password": "secret"}).encode()))
        self.assertEqual(resp.status_code, 401)
...
class TestMe(unittest.TestCase):
    ...
    def test_me_with_valid_token(self):
        username = f"tu_{uuid.uuid4().hex[:8]}"
        resp = _run(handle_register(json.dumps({"username": username, "password": "secret123"}).encode()))
        cookie = resp.headers.get("set-cookie", "")
        token = cookie.split("wc_session=")[1].split(";")[0]

        resp = _run(handle_me(token))
        self.assertEqual(resp.status_code, 200)
        data = json.loads(resp.body)
        self.assertEqual(data["username"], username)

    def test_me_with_invalid_token(self):
        resp = _run(handle_me("invalid_token"))
        self.assertEqual(resp.status_code, 401)

    def test_me_with_none_token(self):
        resp = _run(handle_me(None))
        self.assertEqual(resp.status_code, 401)
